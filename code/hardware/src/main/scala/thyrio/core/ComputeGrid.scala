...
class ComputeGrid(DimX: Int, DimY: Int,
  ...
  val io: ComputeGridInterface = IO(new ComputeGridInterface(ThyrioISA))
  ...
  withClockAndReset(clock, reset) {
    ...
    val noc = Module(new BareNoC(DimX, DimY, ThyrioISA))
...
object GenerateComputeGrid extends App {
  ...
  val init = new ComputeGrid.InitialState(
    makeGrid {
      (_, _) => ComputeGrid.createEquations(ThyrioISA)(rdgen.nextInt)
    },
    makeGrid {
      (x, y) => s"rf_${x}_${y}.dat"
    },
