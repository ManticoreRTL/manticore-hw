...
import memory.{CacheBackInterface, CacheCommand, CacheConfig, CacheFrontInterface}
...
class CacheRequestIntercept extends Module {
  ...
  val io = IO (new CacheRequestInterceptInterface)
  ...
  val rdata_reg = Reg(UInt(CacheConfig.DataBits.W))
