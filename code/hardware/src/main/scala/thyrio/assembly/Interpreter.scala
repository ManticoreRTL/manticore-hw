...
case class Environment(register_file: Array[Int], register_array: Array[Int], memory: Array[Int], var select: Boolean)
    ...
    instruction match {
      ...
      case Or2(rd, rs1, rs2) =>
        env.register_file(rd) =
          clipped(env.register_file(rs1) | env.register_file(rs2))
      ...
      case LocalStore(rs, base, offset) =>
        val addr: Int = clipped(env.register_file(base) + clipped(offset.toInt, 11))
