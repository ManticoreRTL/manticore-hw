...
class ComputeGridOrchestrator(DimX: Int, DimY: Int, config: ISA) extends Module {
  ...
  val exception_handler: Seq[ExceptionHandler] = io.periphery_core.map {
    core =>
  ...
  clock_manager.io.start_request
    ...
    .zip(
      clock_manager.io.done_request
        .slice(0, 4))
    .zip(exception_handler).foreach { case ((_start, _end), handler) =>
  ...
  clock_manager.io.start_request
    .slice(4, 8)
    .zip(
      clock_manager.io.done_request.slice(4, 8))
    .zip(
