...
class UniProcessorCoupledCountersTester extends AnyFlatSpec with Matchers with ChiselScalatestTester{
  ...
  def initEnvironment: (Array[Instruction], Map[String, Register], Interpreter) = {
    ...
    val mux_eq = CustomFunction(Seq.fill(ManticoreBaseISA.DataBits){0xcaca})
