...
class GenericMemory(val ADDRESS_WIDTH: Int,
                    val DATA_WIDTH: Int,
  ...
  val memory = Module(
    if (INIT.isEmpty) new GenericChiselMemory(ADDRESS_WIDTH, DATA_WIDTH)
    else              new GenericVerilogMemoryFromFileWrapper(ADDRESS_WIDTH, DATA_WIDTH, INIT)
  )
...
abstract class AbstractGenericMemory(val ADDRESS_WIDTH: Int, val DATA_WIDTH: Int) extends Module {
  val io = IO(
    new GenericMemoryInterface(
      ADDRESS_WIDTH = ADDRESS_WIDTH,
      DATA_WIDTH = DATA_WIDTH))
...
class GenericChiselMemory(ADDRESS_WIDTH: Int, DATA_WIDTH: Int) extends
  AbstractGenericMemory(ADDRESS_WIDTH, DATA_WIDTH) {
...
class GenericVerilogMemoryFromFileWrapper(
                                   ADDRESS_WIDTH: Int,
                                   DATA_WIDTH: Int,
                                   val INIT: String) extends AbstractGenericMemory(ADDRESS_WIDTH, DATA_WIDTH) {

  val impl = Module(new GenericVerilogMemoryFromFile(ADDRESS_WIDTH, DATA_WIDTH, INIT))
  impl.io.wea := io.wea
  impl.io.web := io.web
  impl.io.clock := clock
...
class GenericVerilogMemoryFromFile(val ADDRESS_WIDTH: Int,
                           val DATA_WIDTH: Int,
                           val INIT: String)
  extends BlackBox(
    Map(
...
class SimpleDualPortMemory(val ADDRESS_WIDTH: Int,
  ...
  abstract class VerilogMemory(params: Map[String, Param]) extends BlackBox(params) {
  ...
  class BRAMLike extends VerilogMemory(
    Map(
      "ADDRESS_WIDTH" -> ADDRESS_WIDTH,
      "DATA_WIDTH" -> DATA_WIDTH,
      "filename" -> INIT
...
object MemGen extends App{
  ...
  new ChiselStage().emitVerilog(new GenericVerilogMemoryFromFileWrapper(11, 16, "array.data"))
}
object BRAM2Kx16 {
  def apply(INIT: String = "") = new GenericMemory(
    DATA_WIDTH = 16, ADDRESS_WIDTH = 11, INIT = INIT)
